...
async function setupSharePointLists() {
  ...
  const siteUrl =
    process.env.SHAREPOINT_SITE_URL || 'https://spi.intranet.bs.ch/JSD/Digital/roadmap-app';
  ...
  try {
    ...
    if (projectsList.created) {
      ...
      await projectsList.list.fields.addText('GeplantUmsetzung', {
        MaxLength: 100,
        Required: false,
      });
      ...
      await projectsList.list.fields.addText('NaechsterMeilenstein', {
        MaxLength: 255,
        Required: false,
      });
    }
    ...
    const categoriesList = await sp.web.lists.ensure(
      SP_LISTS.CATEGORIES,
      'Roadmap Categories',
      100
    );
    ...
    const fieldTypesList = await sp.web.lists.ensure(
    ...
    const teamMembersList = await sp.web.lists.ensure(
