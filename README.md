...
SharePoint-backed roadmap application built with Next.js 14 (pages router), TypeScript, Tailwind CSS, and PM2. It integrates with on-prem SharePoint via resilient fetch fallbacks and supports multiple auth modes (Kerberos, NTLM, FBA).
...
3) **Env**: copy `.env.example` to `.env` and set values. Key vars:
   ...
   - `SP_STRATEGY` (`kerberos`|`onprem`|`fba`)
...
## Auth Modes
- **kerberos**: Browser SPNEGO; proxy sets `x-sp-auth-mode: kerberos`; no `Authorization` headers server-side.
- **onprem (NTLM)**: Server builds NTLM auth (see `utils/spAuth.ts`).
...
- SharePoint auth diagnostics: `npm run ntlm:diag`.
